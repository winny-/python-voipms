...
class FaxConnect(BaseApi):
    """
    Connect for the Fax endpoint.
    """
    def __init__(self, *args, **kwargs):
        """
        Initialize the endpoint
        """
        super(FaxConnect, self).__init__(*args, **kwargs)
        self.endpoint = 'fax'

    def connect_fax(self, fax, account, monthly, setup, minute, **kwargs):
        """
        Connects a specific FAX DID to a specific Reseller Client Sub Account

        :param fax: [Required] FAX DID to be Connected to Reseller Sub Account (Example: 5551234567)
        :type fax: :py:class:`str` or `int`
        ...
        if not isinstance(fax, int):
            raise ValueError("FAX to be canceled and deleted needs to be an int (Example: 5551234567)")
